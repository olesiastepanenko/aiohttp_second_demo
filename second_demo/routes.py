...
def setup_routes(app, base_dir):
    ...
    app.router.add_get("/api/count_posts/{filter}", views.get_count_posts_in_db)
    ...
    app.router.add_get("/api/topic/{topic}/{id}/{volume}", views.get_posts_by_topic_json),
    ...
    app.router.add_get(
        "/api/clicksvscategory/{filter}/{filterData}", analyticsViews.get_clicks_depending_frontend_request
    ),
